...
\begin{figure}
  \centering
  \begin{tikzpicture}[domain=-.3:3,label/.style={
        postaction={decorate,transform shape,
          decoration={markings, mark=at position .5 with \node #1;}}}]
    \draw[<-] (0,3.25) node [left] {$u_*$} -- (0,0) -- (4,0) node [below] {$a$};

    \draw[->,thick,label={[below]{stable}}] plot({\x+1},{e-pow(e,-\x)});
    \draw[->,thick,label={[above]{unstable}}] plot({\x+1},{pow(e,-\x)});
    \draw[dashed] (0,{e}) -- (4,{e});
    \draw[fill] (.7,{e/2}) circle (2pt) node[right]{unstable};
  \end{tikzpicture}
