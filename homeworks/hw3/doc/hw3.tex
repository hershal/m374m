...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \draw[thick,<->] (-2,0) -- (4,0)
      ...
      node[near end,above]{unstable} node[above right]{$u_*=0$};
    \draw[thick,<->] (-1.5,-1.5) -- (3,3)
      node[very near start,above,sloped]{unstable}
      node[near end,above,sloped]{stable} node [above right]{$u_*=h$};
...
\begin{figure}
  ...
  \begin{tikzpicture}[domain=2:4,label/.style={
    ...
    \draw[->,thick,label={[above]{stable}}] plot(\x,{sqrt(\x)}) node[right]{$u_*=\sqrt{\frac{\lambda-b}{a}}$};
    \draw[->,thick,label={[above]{stable}}] plot(\x,{-sqrt(\x)})
        node[right]{$u_*=-\sqrt{\frac{\lambda-b}{a}}$};
    \draw[<-|,thick] (-1,0) -- (2,0) node[above left, very near end]{stable};
    \draw[->,thick] (2,0) -- (4,0) node[above right, at start]{unstable} node[above right]{$u_*=0$};

    \draw[dashed] (2,{sqrt(2)}) -- (2,{-sqrt(2)}) node[below right,midway]{$b$};
