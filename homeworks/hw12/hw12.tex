...
\begin{align*}
  1 &= \frac{c^2\pi}{2} - 0 \\
  \implies c &= \sqrt{\frac{2}{\pi}}. \\
\end{align*}
Thus
\begin{equation*}
  \boxed{y(x) = \sqrt{\frac{2}{\pi}} \sin\left(\sqrt{-\lambda}x\right).}
\end{equation*}
...
\begin{figure}[htp]
  \centering
  \begin{tikzpicture}[x=4cm,y=3cm,scale=0.5]
    \draw[<->] (0,3)node[left]{$y$} -- (0,-3.1);
    \draw[<->] (-3,0) -- (3.1,0)node[below]{$c$};
    \draw[domain=-1.82:1.82] plot({\x}, {(e^(\x)-e^(-\x))/2}) node[left]{$\sinh(c)$};
    \draw[domain=-3:3] plot({\x}, {\x}) node[right]{$c$};
    \draw[domain=-3:3,dotted] plot({\x}, {0.5*\x}) node[right]{$0.5c$};
    \draw[domain=-2:2,dashed] plot({\x}, {1.5*\x}) node[right]{$1.5c$};
    \draw (0,0)node[circle,fill,inner sep=1pt]{};
    \draw (1.62213,2.433195)node[circle,fill,inner sep=1pt]{};
    \draw (-1.62213,-2.433195)node[circle,fill,inner sep=1pt]{};
  \end{tikzpicture}
