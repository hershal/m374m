...
\item If $b=1/2$, then $x_*=1/2$ and $y_*=2/(4a+1)$. We may linearize
  ...
  Or simplified,
  \begin{equation*}
    \lambda^2 - \left(-\frac{5}{4}-a+\frac{2}{a+1}\right)\lambda + \left(a+\frac{1}{4}\right)=0.
  \end{equation*}
  \begin{equation}
    \label{eq:minilab-2-lambda}
    \implies \lambda = \frac{\left(-\frac{5}{4}-a+\frac{2}{a+1}\right)
      \pm\sqrt{{\left(-\frac{5}{4}-a+\frac{2}{a+1}\right)}^2-(4a+1)}}{2}
  \end{equation}
  Observe that
  \begin{equation}
    \label{eq:minilab-2-observation}
    \abs{-\frac{5}{4}-a+\frac{2}{a+1}} >
    \abs{\sqrt{\left(-\frac{5}{4}-a+\frac{2}{a+1}\right)^2-(4a+1)}}
  \end{equation}
  \todo[not sure what to do from here]
  % From \cref{eq:minilab-2-lambda,eq:minilab-2-observation} we know that the
  % solution changes stability between an unstable
