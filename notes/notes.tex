...
\end{equation}
Hence \cref{eq:x-equation,eq:y-equation,eq:v-equation} reduces today
\begin{equation}
  \label{eq:xu-equation}
  \begin{aligned}
    \dot{x} &= -k_1x(x-\lambda) + k_2u, &\quad x(0)&=a \\
    \dot{u} &= k_1x(x-\lambda) - (k_2+k_3)u, &\quad u(0)&=0. \\
  \end{aligned}
\end{equation}
Let $\epsilon=1/k$ which we assume to be ``small'', $\eta=k_2$, and
$\gamma=k_3$. Then
\begin{equation}
  \label{eq:xu-transformed}
  \text{(IVP)}\quad \left\{
    \begin{aligned}
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
Therefore $c_1=a-\lambda$. In general, the leading-order composite approximation
is
\begin{equation*}
