...
\section{\todo{Case Study: Spinning Body}}
...
\section{\todo{Regulary Perturbed Differential Equations}}
\todo{}
...
\section{\todo{Poincare Linstedt Method}}
\todo{}
...
\section{\todo{Singularly Perturbed Differential Equations}}
...
\section{\todo{Case Study: Liquid-Gas Interface}}
...
$c$, and gravity $g$. Reference \cref{fig:slide}. We seek a profile which minimizes the travel
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[x=4cm,y=3cm,label/.style={ postaction={
        decorate,transform shape, decoration={ markings, mark=at position .8
          with \node #1;}}}]
    \draw[<-] (0,1)node[left]{$y$} -- (0,0) -- (0,0);
    \draw[->] (0,0) -- (1.5,0)node[below]{$x$};
    \draw (0,.75)node[circle,fill,inner sep=1pt]{}node[left]{$h$};
    \draw ({.375*(pi-sin(pi r))},0)node[circle,fill,inner sep=1pt]{}node[below]{$\ell$};
    \draw[domain=0:pi,samples=50,pattern=north east lines] (0,0.75) --
    plot({.375*(\x - sin(\x r))},{.75-.375*(1 - cos(\x r))}) -- ({.375*(pi-sin(pi r))},0) -- (0,0) -- cycle;
    \draw[->] (.5,.5)node[above right]{$y(x)$} -- ({.375*(pi/2-sin(pi/2 r))+.01},{.75-.375*(1 - cos(pi/2 r))+.01});
    \draw[->] (-0.5,0.5)node[circle,fill,inner sep=1pt]{}node[above]{$g$} -- (-0.5,0.25);
    \draw[dashed] (-0.125,.875) -- (1.2,.875)node[right]{$h+\frac{c^2}{2g}$};
  \end{tikzpicture}
  \caption{Profile of slide}
  \label{fig:slide}
\end{figure}
