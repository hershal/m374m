...
which the sign becomes positive. Thus $f$ has positive slope. Thereforce $u_*$
...
$E'(u)f(u) = (2u)(-5u^3) = -10u^4 < 0$ for all $u\in(-\infty,\infty)$, $u\ne
u_*$. $u_*$ is asymptotically stable.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
$u_*$ is an equilibrium solution for $h\in(-\infty,\infty)$. $u_*=\pm\sqrt{h}$
is an equilibria for all $h\ge0$. This solution has 1 or 3 equilibria depending
on $h$. Reference fig. 8.1.
...
\item $u_*=0, h\in(-\infty,\infty)$, then $\lambda_*=-h$.
  ...
  unstable if $h=0$.
\item $u_*=+\sqrt{h},\quad h>0$, then $\lambda_*=3u_*^2-h=2h.$
